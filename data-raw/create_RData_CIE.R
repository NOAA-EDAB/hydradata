...
# This is the exact same data set used in the CIE review

create_RData_CIE <- function() {
  path <- "data-raw"
  ...
  hydraDataList_CIE <- d
  hydraDataList_CIE <- modifyList(hydraDataList_CIE,p)
  ...
  usethis::use_data(hydraDataList_CIE,overwrite = TRUE)
...
get_DatData <- function(path){
  ...
  singletons <- read.csv(paste0(path,"/singletons_CIE.csv"),header=FALSE,row.names = 1)
  ...
  stockRecruit <- read.csv(paste0(path,"/recruitment_species_CIE.csv"),header=TRUE,row.names=1)
...
get_PinData <- function(path){
  ...
  fisheryqs<- read.csv(paste0(path,"/fishing_q_CIE.csv"),header=TRUE,row.names = 1)
