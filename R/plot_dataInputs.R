...
plot_Foodweb <- function(outPath) {
  ...
  graphics.off()
  # png(paste0(outPath,"/Hydra_foodWeb.png"),width=8,height=11.5,units="in",res=300)
  # par(mfrow=c(2,1),oma=c(1,4,4,4))
  #
  # visweb(web = foodweb,  prednames = TRUE, preynames = TRUE, labsize = .5,
  #        type = "none", clear = FALSE)
  #
  # plot.igraph(web,edge.arrow.size=.5,vertex.color=V(web)$color,vertex.size= 20* 2^(V(web)$predPrey),
  #             layout=l)
  # legend("bottomleft",legend=guilds$guild,pch=21,
  #        pt.bg=colorVec, pt.cex=2, cex=.8, bty="n", ncol=1)
  #
  # graphics.off()
}
