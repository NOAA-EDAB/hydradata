...
write_DatFile <- function(d,options,listOfParameters) {
  ...
  for (idata in 1:Nspecies){
    cat(c(" ",observedBiomass[idata,]),file=outputFileName,fill=listOfParameters$fillLength,append=TRUE)
  }
  # observed catch
  ...
  for (idata in 1:Nspecies){
    cat(c(" ",observedCatch[idata,]),file=outputFileName,fill=listOfParameters$fillLength,append=TRUE)
  }
  # observed effort
  ...
  for (idata in 1:Nfleets){
    cat(c(" ",observedEffort[idata,]),file=outputFileName,fill=listOfParameters$fillLength,append=TRUE)
  }
